...
def create_tables(cursor)
    Create report_table and main tables if not exist.
...
def clear_existing_data(cursor)
    Truncate all main tables and report_table.
    print([INFO] Clearing existing data...)
    for system_tables in MAIN_TABLES.values()
        for table in system_tables
            cursor.execute(fTRUNCATE TABLE {table} RESTART IDENTITY CASCADE;)
...
def generate_report_entry(external_number)
    Create a single report_table entry with realistic YN flags.
...
def generate_main_table_entries(report_entry)
    Create main table rows based on report_table flags.
    entries = []
    ...
    try
        ...
        for i in range(1, NUM_RECORDS + 1)
            ...
            if i % BATCH_SIZE == 0
                ...
                report_values = [
                    (r['external_number'], r['internal_number'], r['system'], r['isabc'], r['isxyz'], r['isdef'], r['status'])
                ...
                for table, entries in table_grouped.items()
                    cursor.executemany(
                        fINSERT INTO {table} (external_number, internal_number, name, email, phone, system_id) 
                        VALUES (%s,%s,%s,%s,%s,%s),
